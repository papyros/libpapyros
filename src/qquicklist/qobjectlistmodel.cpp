...
bool QObjectListModel::elementChangeTracking() const
{
    return m_tracking;
}
...
void QObjectListModel::timerEvent(QTimerEvent *ev)
{
    if (ev->timerId() != m_notifyTimer.timerId())
        return;
    emit dataChanged(index(m_notifyIndexes.begin().key()), index((m_notifyIndexes.end() - 1).key()),
